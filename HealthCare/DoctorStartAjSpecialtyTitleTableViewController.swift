...
class DoctorStartAjSpecialtyTitleTableViewController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        bcDescriptionLabel.layer.borderColor = UIColor.redColor().CGColor
        ...
        certificationImage.hidden = true
        //setup navigation
        ...
        if signInUser?.doctorImageSpecialistLicense !=  nil{
            labelToUpload(boardCertificationDescription)
            certificationImage.hidden = false
        }
    }
    ...
    private func imageSpecialtyLicense(){
    ...
    private func backButtonClicked(){
        NSNotificationCenter.defaultCenter().postNotificationName("SpecialtyTitleBack", object: self, userInfo: nil )
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.section == 0{
            ...
            index = NSIndexPath(forRow: indexPath.row, inSection: 0)
            cell = tableView.cellForRowAtIndexPath(index)
            cell!.accessoryType = .Checkmark
            tempDoctor?.doctorProfessionTitle = String(indexPath.row)
        }
