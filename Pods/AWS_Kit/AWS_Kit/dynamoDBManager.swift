...
public enum UpdateOp:String{
    case SET = "SET"
    case REMOVE = "REMOVE"
    case ADD = "ADD"
    case DELETE = "DELETE"
}
public enum LogicOp:String{
    case AND = "AND"
    case OR = "OR"
}
public class dynamoDBManger : NSObject {
    let moc:NSManagedObjectContext
    let dynamoDB:AWSDynamoDB
    public init(moc:NSManagedObjectContext) {
        self.moc = moc
        self.dynamoDB = AWSDynamoDB.defaultDynamoDB()
    }
    ...
    public func updateItem(tabName:String,upExp:String,condExp:String,inout dictName:[String : String],inout dictValue:[String : AWSDynamoDBAttributeValue])->AWSDynamoDBUpdateItemInput{
    ...
    public func timeFromDate(date:NSDate)->(hour:Int32,min:Int32){
    ...
    public func testQuery(tabName:String,keyVal:[AnyObject],keyTyp:[NSAttributeType],op:Op)->AWSTask{
    ...
    public func queryItem(tabName:String,keyExp:String,inout dictName:[String : String]?,inout dictValue:[String : AWSDynamoDBAttributeValue]?)->AWSDynamoDBQueryInput {
    ...
    public func query(input:AWSDynamoDBQueryInput)->AWSTask{
    ...
    public func conCat(delit:String,strs:String... )->String{
        var count:Int = 0
        var return_str = ""
        for str in strs{
            if(count==0){
    ...
    public func updateOpExp(op:String,exp:String)->String{
        return op+" "+exp
    }
    //create string for binary operator, add new map to dictName and dictValue
    public func operationBi(left:String,op:Op?,right:AnyObject,type:NSAttributeType,inout dictName:[String : String]?,inout dictValue:[String : AWSDynamoDBAttributeValue]?)->String{
    ...
    public func toAttrValue(attrValue:AWSDynamoDBAttributeValue,type:NSAttributeType,object:AnyObject){
    ...
    public func transTodelete(objects:[String:[NSManagedObject]])->AWSDynamoDBBatchWriteItemInput{
    ...
    public func transTowrite(objects:[String:[NSManagedObject]])->AWSDynamoDBBatchWriteItemInput{
        let keys = Array(objects.keys)
        let returnObject = AWSDynamoDBBatchWriteItemInput()
        returnObject.requestItems = [String : [AWSDynamoDBWriteRequest]]()
        //each key indicate a entity name
    ...
    public func describeTable(tableName:String) -> AWSTask {
    ...
    public func createTable(tableName:String,key:[AWSDynamoDBAttributeDefinition],tableType:Bool,readCap:Int,writeCap:Int) -> AWSTask {
        //Create the table
        ...
        return dynamoDB.createTable(createTableInput).continueWithSuccessBlock({ task -> AnyObject? in
            ...
            if ((localTask.result) != nil) {
                ...
                let describeTableInput = AWSDynamoDBDescribeTableInput()
                describeTableInput.tableName = tableName;
                localTask = self.dynamoDB.describeTable(describeTableInput)
                ...
                for _ in 0...15 {
                    localTask = localTask.continueWithSuccessBlock({ task -> AnyObject? in
                        ...
                        sleep(15)
                        return self.dynamoDB .describeTable(describeTableInput)
                    })
                }
            }
